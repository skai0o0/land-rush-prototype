...
def _slab_block(name, cx, cy, length, depth, floors, mat_wall, mat_balcony, mat_window, collection, rot_z=0.0):
    ...
    for fl in range(floors):
        ...
        for sign in (-1.0, 1.0):
            ...
            objs.append(win)
            # balcony rail posts (every other floor to stay under 250KB)
            if fl % 2 == 0:
                for j in range(3):
                    px = cx - length * 0.25 + j * (length * 0.5 / 2)
                    if abs(rot_z) > 0.01:
                        lx = -length * 0.25 + j * (length * 0.5 / 2)
...
def build(collection):
    ...
    for i, (x, y) in enumerate([(-10, -7), (10, -7), (-10, 7), (10, 7), (-14, 0), (14, 0)]):
        ...
        if i % 2 == 0:
            canopy2 = lm.add_icosphere(f"KTXA_Canopy2_{i}", 1.0, (x + 0.5, y - 0.3, 4.3), mat_leaf, collection, subdivisions=1)
            objs.append(canopy2)
        objs.extend([trunk, canopy])
